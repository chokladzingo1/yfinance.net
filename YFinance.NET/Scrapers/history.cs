...
namespace yfinance.scrapers
{
    public class PriceHistory
    {
        ...
        public PriceHistory(YfData data, string ticker, string tz, object session = null, IWebProxy proxy = null)
        {
            _data = data;
        ...
        {
            // Logging
            //var logger = null; //Utils.GetYfLogger();
            ...
            try
            {
                jsonStr = _data.GetRawJson(url, paramsDict).GetRawText();
            }
        ...
        public JsonElement? GetHistoryMetadata(IWebProxy proxy = null)
        {
            if (proxy != null && !proxy.Equals(Consts.SENTINEL))
            {
                Debug.WriteLine("Set proxy via new config function: yf.set_config(proxy=proxy)");
