    ...
    public class Quote
    {
        ...
        public DataTable Sustainability
        {
            get
            {
                ...
                {
                    var result = Fetch(new List<string> { "esgScores" });
                    if (result == null)
        ...
        public DataTable Recommendations
        {
            get
            {
                ...
                {
                    var result = Fetch(new List<string> { "recommendationTrend" });
                    if (result == null)
            ...
            var paramsDict = new Dictionary<string, string>
        {
            ...
            try
            {
                string json = _data.GetRawJson($"{Consts.BASE_URL}/v10/finance/quoteSummary/{_symbol}", paramsDict);
            ...
            var paramsDict = new Dictionary<string, string>
        {
            ...
            try
            {
                string json = _data.GetRawJson($"{Consts.QUERY1_URL}/v7/finance/quote?", paramsDict);
                var doc = JsonDocument.Parse(json);
                return doc.RootElement;
            }
            catch (HttpRequestException)
